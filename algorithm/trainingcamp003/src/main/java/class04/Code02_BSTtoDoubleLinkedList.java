...
public class Code02_BSTtoDoubleLinkedList {
    ...
    public static Node convert1(Node head) {
        ...
        inOrderToQueue(head, queue); // 中序遍历依次入队列
        if (queue.isEmpty()) {
            return head;
        }
        head = queue.poll();
    ...
    public static Info process(Node X) {
        ...
        if (leftInfo.end != null) {
            leftInfo.end.right = X; // 左树的结束节点的右指针(next)指向X节点
        }
        X.left = leftInfo.end; // X节点的左指针(pre)指向左树的结束节点
        X.right = rightInfo.start; // X节点的右指针(next)指向右树的开始节点
        if (rightInfo.start != null) {
            rightInfo.start.left = X; // 右树的开始节点的左指针(pre)指向X节点
        }
        return new Info(
                // 整棵树的头，
