...
public class Code02_ExpressionNumber {
     ...
     * 无组合则可以得到false，返回0
     */
    // 校验字符串是否合法，不合法，返回0种组合
    // 1）字符串长度必须为奇数，否则不合法
    // 2）偶数位上不是1就是0，奇数位上必须是运算符
    public static boolean isValid(char[] exp) {
        if ((exp.length & 1) == 0) { // 校验字符串长度
            return false;
        }
        // 偶数位校验
    ...
    public static int dpLive(String express, boolean desired) {
        ...
        int N = str.length;
        int[][] tMap = new int[N][N]; // true dp表
        int[][] fMap = new int[N][N]; // false dp表
        for (int i = 0; i < N; i += 2) { // 斜对角线上的格子
            tMap[i][i] = str[i] == '1' ? 1 : 0;
            fMap[i][i] = str[i] == '0' ? 1 : 0;
        }
        for (int row = N - 3; row >= 0; row -= 2) { // 从倒数第三行开始 从下往上
            for (int col = row + 2; col < N; col += 2) { // 从左往右
                // row..col tMap fMap
