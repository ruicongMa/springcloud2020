...
public class Code01_DeleteMinCost {
    ...
    public static int minCost3(String s1, String s2) {
        ...
        int[][] dp = new int[M][N];
        int ans = M;
        for (int start = 0; start < N; start++) { // 开始的列数，之前列不用管
            dp[0][start] = str2[0] == str1[start] ? 0 : M;
            for (int row = 1; row < M; row++) {
                dp[row][start] = (str2[row] == str1[start] || dp[row - 1][start] != M) ? row : M;
            }
            ans = Math.min(ans, dp[M - 1][start]); // str2[0...M-1]整体删除多少个字符达到str1一个字符的子串
            // 以上把dp[...][start...start]的信息填好，str1中的子串长度为1（一个字符）
    ...
    public static void main(String[] args) {
        ...
        String str1 = "bbecda";
        String str2 = "bcdead";
        System.out.println(minCost2(str1, str2));
    }
}
