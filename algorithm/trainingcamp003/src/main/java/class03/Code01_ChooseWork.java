...
public class Code01_ChooseWork {
     ...
     * }
     * 给定一个Job类型的数组jobarr，表示所有岗位，每个岗位都可以提供任意份工作，选工作的标准是在难度不超过自身能力值的情况下，选择报酬最高的岗位
     * 给定一个int类型的数组arr，表示所有人的能力，返回int类型的数组，表示每个人按照标准选工作后所能获得的最高报酬
     */
    ...
    public static int[] getMoneys(Job[] job, int[] ability) {
        ...
        for (int i = 0; i < ability.length; i++) {
            Integer key = map.floorKey(ability[i]); // ≤ 能力的最大报酬
